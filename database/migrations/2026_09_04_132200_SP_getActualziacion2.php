...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        DB::unprepared('drop procedure if exists getActualizacionesCabecera2');

        DB::unprepared("
        CREATE PROCEDURE `getActualizacionesCabecera2`(
IN `p_idSucursal` BIGINT(20),
IN `p_motivo_codigo_filter` varchar(20),
IN `p_motivo_codigo_exclude` varchar(20)
)
BEGIN
  ...
  SELECT * FROM (
    SELECT
      ...
      WHERE
          ...
          vrsa.cerrada = 1 AND vrs.idmotivoactualizacion <> 18 and
          (p_motivo_codigo_filter is null or p_motivo_codigo_filter = ma.codigo) and
          (p_motivo_codigo_exclude is null or not p_motivo_codigo_exclude = ma.codigo)
      GROUP BY
          suc.nombre ,
          suc.descripcion ,
    ...
    UNION ALL
     ...
     WHERE
        ...
        (am.fecha > aa.fecha OR aa.fecha IS NULL) and
        (p_motivo_codigo_filter is null or p_motivo_codigo_filter = 18) and
       (p_motivo_codigo_exclude is null or not p_motivo_codigo_exclude = 18)
      GROUP BY
        am.idsucursal,
        suc.nombre,
        suc.descripcion
