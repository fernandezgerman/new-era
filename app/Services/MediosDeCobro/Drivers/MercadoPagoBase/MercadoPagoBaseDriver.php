...
class MercadoPagoBaseDriver implements MedioDeCobroQRDriverInterface, MedioDeCobroEventHandlerInterface
{
    protected HttpClient $httpClient;
    protected MercadoPagoExtendedFunctionalities $extendedFunctionalities;
    ...
    public function __construct(protected ConnectionDataDTO $connectionDataDTO)
    {
        ...
        $connectionDataDTO->host = config('medios_de_cobro.drivers.'.$connectionDataDTO->modoDeCobro->driver.'.host_extended_functionalities');
    ...
    protected function getMercadoPagoQROrderSqlByLocalId(string $localId): MercadoPagoQROrderSql
    {
    ...
    protected static function getModoDeCobroStatus(string $mpQrStatus): MedioDeCobroEstados
    {
    ...
    public function testConnection(int $sucursalId): bool
    {
        ...
        return $this->testConnectionByDriver($sucursalId, $this->connectionDataDTO->modoDeCobro->driver);
        ...
        try {
            ...
            $mercadoPagoCajaDTO = $this->extendedFunctionalities->getOrCreateCajaForSucursal($configuracionPorSucursal);
        } catch (\Throwable $throwable) {
            Log::error($throwable->getMessage());
            throw $throwable;
        }
    ...
    protected function testConnectionByDriver(int $sucursalId): bool{
        $mercadoPagoCajaDTO = null;
        try {
            // 1- test token
            $this->httpClient->get('orders/AAAA1');
